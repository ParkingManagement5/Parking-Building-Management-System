...
public class BookingServiceImpl implements BookingService {
    ...
    private BigDecimal calculateDeposit(String vehicleTypeName, long minutesUntilStart) {
        // BR-03d: chỉ CAR và ELECTRIC_CAR mới tính cọc
        if (!vehicleTypeName.equals("CAR") && !vehicleTypeName.equals("ELECTRIC_CAR")) {
            return BigDecimal.ZERO;
        }
        if (!vehicleTypeName.equalsIgnoreCase("Car")
                && !vehicleTypeName.equalsIgnoreCase("Electric Car")) {
            return BigDecimal.ZERO;
        }
