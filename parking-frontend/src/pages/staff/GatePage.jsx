...
export default function MapPage() {
  ...
  async function loadInitial() {
    ...
    try {
      ...
      setLoadError(err.response?.data?.message || "Không tải được bảng giá.");
    }
  ...
  async function loadFloors() {
    ...
    try {
      ...
      setLoadError(items.length ? "" : "Bãi xe chưa có tầng nào để hiển thị.");
    } catch (err) {
      setLoadError(err.response?.data?.message || "Không tải được danh sách tầng.");
    }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        {/* LEFT: Parking Map */}
        <StaffPageSection title="Bản đồ bãi xe" subtitle={assignedLabel || `Bãi #${assignedId}`}>
          {/* Floor tabs */}
